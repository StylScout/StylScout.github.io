...
import { ShoppingBag, LogOut, Menu, X, Search } from 'lucide-react';
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between items-center h-16">
            ...
            <nav className="hidden md:flex space-x-8 items-center">
              ...
              ` : html`
                <div className="flex items-center gap-3 pl-4 border-l border-gray-200">
                  <${Link} to="/login">
                    <${Button} variant="outline" size="sm" className="border-gray-300 text-gray-700">Log In<//>
                  <//>
                  <${Link} to="/signup">
                    <${Button} variant="primary" size="sm" className="bg-fashion-900 hover:bg-gray-800">Sign Up<//>
                  <//>
                </div>
              `}
            </nav>
            ...
            <div className="flex items-center md:hidden">
              ...
              >
                ${isMobileMenuOpen ? html`<${X} size=${24} />` : html`<${Menu} size=${24} />`}
              </button>
            </div>
          </div>
        </div>
              ...
              ` : html`
                <div className="grid grid-cols-2 gap-3 p-3">
                  <${Link} to="/login" onClick=${() => setIsMobileMenuOpen(false)}>
                    <${Button} variant="outline" className="w-full justify-center">Log In<//>
                  <//>
                  <${Link} to="/signup" onClick=${() => setIsMobileMenuOpen(false)}>
                    <${Button} variant="primary" className="w-full justify-center bg-fashion-900">Sign Up<//>
                  <//>
                </div>
              `}
            </div>
